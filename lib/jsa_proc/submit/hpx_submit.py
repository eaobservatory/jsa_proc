...
def submit_one_coadd_job(tile, parenttask, mode, parameters, location,
    ...
    try:
        ...
        logger.debug('Coadd for tile %i is in already-existing job %i' % (
            tile, oldjob.id))
        if never_update:
            raise JSAProcError(
    ...
    if oldparents:
        ...
        if set(pars) != set(oldspars) or set(oldfilts) != set(filts):
            ...
            logger.debug('Parent jobs %s have been removed from coadd.' % str(
                removed_jobs))
            logger.debug(
                'Parent jobs %s have been added to coadd.' % str(added_jobs))
...
def get_parents(tile, parenttask, exclude_pointing_jobs=False,
    ...
    logger.debug('Finding all jobs in task %s that fall on tile %i' % (
        parenttask, tile))
    ...
    if len(excludedjobs_ompstatus) > 0:
        ...
        for i in excludedjobs_ompstatus:
            ...
            logger.debug('Job %i NOT INCLUDED (omp status of %s)' % (
